...
class DDPG_agent():
	def __init__(self, **kwargs):
		...
		self.replay_buffer = ReplayBuffer(self.state_dim, self.action_dim, max_size=int(1024), dvc=self.dvc)
	...
	def train(self):
		...
		with torch.no_grad():
			...
			mask = 1 - dw.float()  # 当 dw 为 True 时 mask 为 0，False 时为 1
			target_Q = r + mask * self.gamma * target_Q
		...
		with torch.no_grad():
			for param, target_param in zip(self.actor.module.parameters(), self.actor_target.parameters()):
				target_param.data.copy_(self.tau * param.data + (1 - self.tau) * target_param.data)

			for param, target_param in zip(self.q_critic.module.parameters(), self.q_critic_target.parameters()):
				target_param.data.copy_(self.tau * param.data + (1 - self.tau) * target_param.data)
